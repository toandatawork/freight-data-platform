...
with
    ...
    trip_dispatch as (
        select load_id, dispatch_date as dispatched_at, trip_id
        from {{ ref("stg_freight__trips") }}
    ),
    trip_fuel_cost as (
        select t.load_id, sum(fp.total_cost) as fuel_cost
        from {{ ref("stg_freight__trips") }} t
        join {{ ref("stg_freight__fuel_purchases") }} fp on t.trip_id = fp.trip_id
        group by 1
    )
...
left join trip_dispatch td on l.load_id = td.load_id
left join pickup_events pe on l.load_id = pe.load_id
left join delivery_events de on l.load_id = de.load_id
left join trip_fuel_cost tf on l.load_id = tf.load_id
